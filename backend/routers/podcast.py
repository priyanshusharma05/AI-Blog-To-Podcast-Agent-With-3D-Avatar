...
async def generate_episode(request: Request, background_tasks: BackgroundTasks):
    """
    Full AI pipeline:
    1. Fetch/validate source content (URL scrape OR raw text OR .txt upload)
    2. Clean and chunk the text
    3. Generate podcast script via Gemini
    ...
    if not final_script:
        raise HTTPException(
            status_code=500,
            detail="Script generation returned empty. Check your Gemini configuration.",
        )
